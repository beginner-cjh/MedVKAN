...
class KANBlock(nn.Module):
    def __init__(self, dim, drop=0., drop_path=0., act_layer=nn.SiLU, norm_layer=nn.LayerNorm, no_kan=False,idx=0):
        super().__init__()
    ...
    def forward(self, x, H, W):
        ...
        if self.idx==0:
            ...
            xr = xr.reshape(B,H,W,C).transpose(1,2).reshape(B,-1,C)
            xr = self.layer(xr, H, W)
...
class VKANBlock(nn.Module):
    def __init__(self, dim,attn_drop_rate=0,
                 ...
                 d_state = 16,drop_rate=None,depth = 0,idx=0):
        super().__init__()
        self.ln = norm_layer(dim)
        self.EFConv = EFConv( patch_size=3, stride=1, in_chans=dim,embed_dim=dim)
        self.drop_path = DropPath(drop_path)
        ...
        self.KAN = KANBlock(dim=dim,drop=drop_rate, drop_path=drop_path, norm_layer=norm_layer,idx=idx)
...
class UVKANDecoder(nn.Module):
    def __init__(
        ...
        self.dblock5 = VKBlock(dim=channels[1],
                    drop_path=dpr[0],
                    norm_layer=norm_layer,
                    attn_drop_rate=attn_drop_rate,
                    drop_rate=drop_rate,
        ...
        self.dblock4 = VKBlock(dim=channels[2],
                    drop_path=dpr[1],
                    norm_layer=norm_layer,
                    attn_drop_rate=attn_drop_rate,
                    drop_rate=drop_rate,
        ...
        for s in range(3):# for deep supervision
            input_features_skip = channels[3-s]
...
class UVKAN(nn.Module):
    def __init__(self, encoder_args, decoder_args):
        super().__init__()
        self.encoder = UVKANEncoder(**encoder_args)
        self.decoder = UVKANDecoder(**decoder_args)
...
def get_u_vkan_from_plans(
    ...
    model = UVKAN(encoder_args,decoder_args)
